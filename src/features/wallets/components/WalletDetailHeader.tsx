  ...
  return (
    <View style={styles.container}>
      ...
      <Surface style={styles.card} mode="flat">
        <Text style={styles.balanceTitle}>{t('wallets.balanceTitle', 'Wallet Balance')}</Text>
        <Text style={styles.balanceAmount}>
          {isHidden ? '••••••' : `${formatMoney(balance)} ${currency}`}
        </Text>
        ...
        <View style={styles.statsInlineRow}>
          <View style={styles.statColumn}>
            ...
            <Text style={styles.incomeValue}>
              {isHidden ? '•••' : `+${formatMoney(income30Days)}`}
            </Text>
          </View>
          ...
          <View style={styles.statColumn}>
            ...
            <Text style={styles.expenseValue}>
              {isHidden ? '•••' : `-${formatMoney(expense30Days)}`}
            </Text>
          </View>
        </View>
      </Surface>

      <Text style={styles.historyTitle}>{t('wallets.historyTitle', 'Wallet History')}</Text>
    </View>
  );
}
